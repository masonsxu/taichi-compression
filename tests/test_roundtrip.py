...
class TestQuantRoundtrip(unittest.TestCase):
    """权重量化 × 容器版本：float16 走 v2 容器，float32 保持 v1 兼容。"""

    @classmethod
    def setUpClass(cls):
        try:
            cls.fp16 = LLMPredictor(
                PredictorConfig(model_id=MODEL_ID, device=DEVICE, quant="float16")
            )
            cls.fp32 = LLMPredictor(
                PredictorConfig(model_id=MODEL_ID, device=DEVICE, quant="float32")
            )
    ...
    def test_quant_mismatch_raises(self):
        data = compress_text("Hello World", self.fp16)
        with self.assertRaises(ValueError):
